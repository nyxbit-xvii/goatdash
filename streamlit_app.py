...
STATIC_W, STATIC_H, STATIC_ZOOM = 800, 600, 18  # smaller keeps memory/CPU low
...
def render_mp4(interp: pd.DataFrame, center_lat, center_lon,
    ...
    """Stream frames to FFmpeg to keep memory low."""
    ...
    # Open an FFmpeg writer and write frames one-by-one (no huge list in RAM)
    with iio.imopen(
        outfile, "w", plugin="FFMPEG",
        fps=fps, codec="libx264",  # widely-compatible
        pix_fmt="yuv420p"          # ensures playback on most devices
    ) as writer:
        for i in range(0, len(interp), step):
            base = bg.copy()
            draw = ImageDraw.Draw(base, "RGBA")

            # short trail (~100 prior points at current step)
            if i > 1:
            ...
            draw.rectangle([(10, h - 40), (340, h - 10)], fill=(0, 0, 0, 140))
...
# 2) Inspect a Time — slider drives Bonus + weather rule
st.subheader("2) Inspect a Time — Bonus moves to the selected hour")

# Use 'h' (not 'H') to quiet pandas FutureWarning
...
# Nearest frame using pandas (tz-safe) to avoid timezone warnings
...
# 3) Weekly Timelapse (MP4)
st.subheader("3) Weekly Timelapse (downloadable MP4)")
make_mp4 = st.button("🎬 Generate MP4")
if make_mp4:
    with st.spinner("Rendering timelapse video…"):
        # downsample to every 10th minute and keep 800x600 to stay light
        path = render_mp4(interp, mid_lat, mid_lon, step=10, w=STATIC_W, h=STATIC_H, fps=10)
    # Preview player
    with open(path, "rb") as f:
        data = f.read()
        st.video(data)  # inline preview
        st.download_button("⬇️ Download bonus_week.mp4", data=data, file_name="bonus_week.mp4", mime="video/mp4")
